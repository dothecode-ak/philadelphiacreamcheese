...
{
    ques: "How much <br> time do <br>you have?",
    options: ["Not so <br> much", "A bit <br> more"]
}];
...
function updateNextData() {
    ...
    if (pos < ques_len) {
        ...
        let not_two_much = checkAnserList.includes('Not so <br> much');
        let a_bit_more = checkAnserList.includes('A bit <br> more');
...
$('#nextClick').click(function () {
    ...
    if (!radios[0].checked && !radios[1].checked) {
        ...
        $('.error').html('Please select the option.');
    }
        ...
        $('.progressImg>img').attr('src', 'assets/screen2/Progress_02.png');
...
$('#backClick').click(() => {
    ...
    $('button').css('color', '#fff');
    checkAnserList.pop();
    $('.progressImg>img').attr('src', 'assets/screen2/Progress_01.png')
});
$('#backButtonIns').click(() => {
    vdo1.pause();
    vdo3.pause();
...
$('#finalScreenButton').click(function () {
    ...
    $('#instructionDiv').hide();
    $('#finalScreen').show();
    $('#cuurent_vdo_link').attr('src', 'unset');
